...
Mailbox.prototype.listChildren = function(path, all, callback){
	...
	console.log("LIST COMMAND: " + command+" "+this.client._escapeString(this.client._rootPath) + " " + path + suffix);
...
Mailbox.prototype.listSubscribed = function(path, xinfo, callback){
    ...
    xinfo = xinfo || [];
	console.log("LSUB COMMAND: " + this.client._escapeString(this.client._rootPath)+" "+path);
...
function detectMailboxType(mailboxName){
    ...
    // if(mailboxNames.inbox.indexOf(mailboxName)>=0){
    //     return "INBOX";
    // }
