...
function IMAPClient(port, host, options){
    ...
    // Set idle Noop frequency based on option parameters
    if(this.options.noopFrequency) {
	this.IDLE_TIMEOUT = this.options.noopFrequency * 1000;
    }
    /**
     * Server hostname
     */
    ...
    if(typeof this.options.auth && typeof this.options.auth.XOAuth2){
        if(typeof this.options.auth.XOAuth2 == "object" && typeof this.options.auth.XOAuth2.getToken == "function"){
            ...
            if(!this.options.auth.user && this.options.auth.XOAuth2.user){
                this.options.auth.user = this.options.auth.XOAuth2.user;
             }
 ...
 */
IMAPClient.prototype.IDLE_TIMEOUT = 5* 60 * 1000; // 15 minutes should be around 30 min
...
/**
 * DK: Timeout for commands
 */
 IMAPClient.prototype.COMMAND_TIMEOUT = 60 * 1000; // 60 Secs
...
IMAPClient.prototype._init = function(){
    ...
    /**
     * DK: Timeout to wait for a successful command response from the server
     */
    this._commandTimeout = false;
...
IMAPClient.prototype.connect = function(){
    console.log("client.js: connecting to " + this.options.auth.user);
...
IMAPClient.prototype._onConnect = function(){
    ...
    if(this._connection.socket) {
	     this._connection.socket.on("error",this._onSocketError.bind(this));
    } else {
       this._connection.on("error",this._onSocketError.bind(this));
    }
...
IMAPClient.prototype._onData = function(chunk){
    ...
    if(this._currentMode == this.modes.COMMAND){
        if((match = data.match(/\r?\n/))){ // find the line ending
            ...
            if(this.debug){
                console.log("SERVER: "+line + "(" + this.options.auth.user + ")" );
            }
...
IMAPClient.prototype._onSocketError = function(error){
    this.emit("error", error);
};
...
/**
 * When the command response is not received in COMMAND_TIMEOUT time,
 * emit an error and close the socket
 */
IMAPClient.prototype._handleCommandTimeout = function(){
    var error = new Error("Timeout waiting for a command response");
    error.errorType = "TimeoutError";
    this.emit("error", error);
    this._close();
};
...
IMAPClient.prototype._responseRouter = function(data){
    ...
    //console.log("****RAW DATA: " + inspect(data,false,100));
    ...
    if(data[0]=="*"){
        switch(data[1]){
        	case "STATUS":
                this._handlerUntaggedStatus(data.slice(2));
                return;
            case "CAPABILITY":
        ...
        if(!isNaN(data[1]) && data[2] == "EXISTS"){
            console.log("this._selectedMailbox.count: " + this._selectedMailbox.count + " data[1]:" + data[1]);
            if(this._selectedMailbox.count != Number(data[1])){
                this._selectedMailbox.count = Number(data[1]) || this._selectedMailbox.count || 0;
                console.log("this.idling: " + this.idling + " this.nooping: " + this.nooping);
...
IMAPClient.prototype._processCommandQueue = function(){
    ...
    if(this.debug){
        console.log("CLIENT: "+ (command.data || "").trim() + " (" + this.options.auth.user + ")" );
    }
...
IMAPClient.prototype._handlerTaggedCapability = function(status){
    if(status == "OK"){
        ...
        var error = new Error("Invalid capability response:"+this.options.auth.user);
        error.errorType = "ProtocolError";
        error.errorLog = this._log.slice(0, this._logLength);
        this.emit("error", error);
        this._close();
...
IMAPClient.prototype._handlerTaggedStartTLS = function(status){
    if(status == "OK"){
	    ...
	    console.log("TLS SENDING CAPABILITY");
            this._send("CAPABILITY", this._handlerTaggedCapability.bind(this));
        }).bind(this));
    }else{
    ...
    if(status == "OK"){
        ...
        if(this._xoauth2 && this._xoauth2UntaggedResponse && this._xoauth2RetryCount && this._xoauth2RetryCount < 3){
            this._xoauth2.generateToken((function(err){
                ...
                if(err){
                    ...
                    if(this.listeners('error').length == 0){
                        console.log("No listener for error any more:"+error.toString()+"  "+this.options.auth.user);
                        return;
                     }
                    this.emit("error", error);
                    return;
            ...
            if(this._connection == false) {
              console.log("CONNECTION IS GONEEEEE");
              //this.emit("error",new Error("AuthenticationError"));
              return;
            }
...
IMAPClient.prototype._handlerTaggedStatus = function(callback, status, params){
    callback(this._status,status);
    return;
};
...
IMAPClient.prototype._handlerUntaggedStatus = function(list){
    this._status = list;
};
...
IMAPClient.prototype._handlerUntaggedList = function(list){
    ...
    var tags = list.shift() || [];
    var delimiter = list.shift() || this._mailboxDelimiter;
    var temp = (list.shift() || "");
    if(typeof temp != 'string') {
        console.log("Skipping: " + inspect(originalList,false,100));
        return;
    }
    var path = temp.substr(this._rootPath.length),
...
IMAPClient.prototype._handlerUntaggedLsub = function(list){
    ...
    var path = temp.substr(this._rootPath.length);
    var name = delimiter?path.split(delimiter).pop():path;
    ...
    // else if(detectMailboxType(name) == "INBOX") {
    //   mailbox.type="Inbox";
    // }
...
IMAPClient.prototype._idleTimeout = function(){
    var user;
    if(this.options.auth.XOAuth2){
        user = this.options.auth.XOAuth2.user;
    }else{
        user = this.options.auth.user;
    }
	//var user = this.options.auth.XOAuth2.user || this.options.auth.user;
	console.log("*************IDLE_TIMEOUT************: " + user);
    this._send("NOOP", this.idle.bind(this));

    // DK
    //this._commandTimeout = setTimeout(this._handleCommandTimeout.bind(this), this.COMMAND_TIMEOUT);
                ...
                this._send("AUTHENTICATE XOAUTH2 " + token,
                     this._handlerTaggedLogin.bind(this), (function(){
                        ...
                        this._literalHandler = (function(message){
                            ...
                            if(this.debug){
                                console.log("CLIENT:" + " (" + this.options.auth.user + ")" );
                            }
                        }).bind(this);
                     }).bind(this));
            }).bind(this));
...
IMAPClient.prototype._formatEnvelopeAddress = function(address){
    ...
    if(name && typeof name == 'string'){
        ...
        str = "";
        console.log("name:"+inspect(name,false,100));
        console.log("email:"+inspect(email,false,100));
    }

    return {
        name: str.replace(/\=\?[^?]+\?[QqBb]\?[^?]+\?=/g,
                function(mimeWord){
                    return mimelib.decodeMimeWord(mimeWord);
                }),
        address: email
    ...
    if (!Array.isArray(bs[0]) && typeof bs[0] === 'string' && bs.length >= 10) {
        ...
        if (bs[2]) {
            ...
            while(parametersIndex < bs[2].length) {
                if(typeof bs[2][parametersIndex] === 'string' && typeof bs[2][parametersIndex+1] === 'string'){
        ...
        if(typeof bs[5] === 'string'){
            currentPart.encoding = bs[5].toLowerCase();
        }else{
            currentPart.encoding = "base64";
        }
...
IMAPClient.prototype._checkNewMail = function(){
    ...
    this._send("UID FETCH " + this._selectedMailbox.UIDNext + ":* (UID BODYSTRUCTURE FLAGS ENVELOPE INTERNALDATE BODY.PEEK[HEADER]" +
...
IMAPClient.prototype.status = function(path,callback) {
	...
	this._send(command + " " + this._escapeString(path)+ " (UNSEEN)" +(
        ""
      ), this._handlerTaggedStatus.bind(this, callback));
...
IMAPClient.prototype.listMessages = function(from, limit, extendedOptions, callback){
    ...
    this._send(
      "FETCH " + from + ":" + to +
      " (UID BODYSTRUCTURE FLAGS ENVELOPE INTERNALDATE BODY.PEEK[HEADER]" +
      (this._capabilities.indexOf("X-GM-EXT-1")>=0?" X-GM-LABELS X-GM-THRID":"") +
      (this._capabilities.indexOf("CONDSTORE") >= 0 ? " MODSEQ" : "") +
      ")" +
      (extendedOptions ? " "+extendedOptions : ""), (function(status){
        ...
        if(status == "OK"){
            ...
            // DK Change: Handle NO Response - We will assume this is a warning only.  Just log it
            callback(null,this._mailList);
        }else{
...
IMAPClient.prototype.createMessageStream = function(uid){
    ...
    //this._send("UID FETCH " + uid + " (UID FLAGS RFC822.SIZE BODYSTRUCTURE INTERNALDATE ENVELOPE BODY.PEEK[HEADER.FIELDS (References)])", (function(status){
 ...
 * @param {Number} uid Message identifier
 */
IMAPClient.prototype.createMessageStreamFast = function(uid){
    var stream = new Stream();
    ...
    //this._send("UID FETCH " + uid + " BODY.PEEK[]", (function(status){
    this._send("UID FETCH " + uid + " (UID FLAGS RFC822.SIZE BODYSTRUCTURE INTERNALDATE ENVELOPE BODY.PEEK[HEADER])", (function(status){
    ...
    if(this._capabilities.indexOf("IDLE")>=0){
        ...
        this._idleTimer = setTimeout((function(){
            this._send("NOOP", (function(){
                this.nooping = true; // DK Change Oct 25, 2015 this.nooping = false;
            }).bind(this), (function(){
                this.nooping = true;
            }).bind(this));
...
IMAPClient.prototype._close = function(){
    ...
    if(this.debug){
        console.log("Connection to server closed: " + this.options.auth.user);
    }
